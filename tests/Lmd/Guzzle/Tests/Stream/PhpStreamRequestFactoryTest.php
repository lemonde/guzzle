...
namespace Lmd\Guzzle\Tests\Stream;

use Lmd\Guzzle\Stream\Stream;
use Lmd\Guzzle\Stream\PhpStreamRequestFactory;
use Lmd\Guzzle\Http\Client;

/**
 * @group server
 * @covers \Lmd\Guzzle\Stream\PhpStreamRequestFactory
 */
class PhpStreamRequestFactoryTest extends \Lmd\Guzzle\Tests\GuzzleTestCase
{
    /** @var Client */
    protected $client;
    ...
    {
        $request = $this->client->get('/');
        $this->factory = $this->getMockBuilder('Lmd\Guzzle\Stream\PhpStreamRequestFactory')
    ...
    public function testCanCreateCustomStreamClass()
    {
        $this->getServer()->enqueue("HTTP/1.1 200 OK\r\nContent-Length: 2\r\n\r\nhi");
        $request = $this->client->get('/');
        $stream = $this->factory->fromRequest($request, array(), array('stream_class' => 'Lmd\Guzzle\Http\EntityBody'));
        $this->assertInstanceOf('Lmd\Guzzle\Http\EntityBody', $stream);
    }
}
