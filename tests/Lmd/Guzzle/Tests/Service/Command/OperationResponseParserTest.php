...
use Lmd\Guzzle\Service\Command\OperationResponseParser;
...
use Lmd\Guzzle\Service\Command\LocationVisitor\Response\StatusCodeVisitor;
use Lmd\Guzzle\Service\Command\LocationVisitor\Response\ReasonPhraseVisitor;
use Lmd\Guzzle\Service\Command\LocationVisitor\Response\JsonVisitor;
use Lmd\Guzzle\Service\Command\LocationVisitor\Response\BodyVisitor;
use Lmd\Guzzle\Service\Command\LocationVisitor\VisitorFlyweight;

/**
 * @covers \Lmd\Guzzle\Service\Command\OperationResponseParser
 * @covers \Lmd\Guzzle\Service\Command\CreateResponseClassEvent
 */
class OperationResponseParserTest extends \Lmd\Guzzle\Tests\GuzzleTestCase
{
    ...
    public function testSkipsUnkownModels()
    {
        ...
        $op->setResponseParser($parser)->setClient(new Client());
        $op->prepare()->setResponse(new Response(201), true);
        $this->assertInstanceOf('Lmd\Guzzle\Http\Message\Response', $op->execute());
    }
    ...
    public function testAllowsModelProcessingToBeDisabled()
    {
        ...
        $result = $op->execute();
        $this->assertInstanceOf('Lmd\Guzzle\Service\Resource\Model', $result);
        $this->assertEquals(array(
            'baz'    => 'bar',
            'enigma' => '123'
        ), $result->toArray());
    ...
    public function testCreatesCustomResponseClassInterface()
    {
        $parser = OperationResponseParser::getInstance();
        $description = ServiceDescription::factory(array(
            'operations' => array('test' => array('responseClass' => 'Lmd\Guzzle\Tests\Mock\CustomResponseModel'))
        ));
        $operation = $description->getOperation('test');
        $op = new OperationCommand(array(), $operation);
        $op->setResponseParser($parser)->setClient(new Client());
        ...
        $result = $op->execute();
        $this->assertInstanceOf('Lmd\Guzzle\Tests\Mock\CustomResponseModel', $result);
        $this->assertSame($op, $result->command);
    }

    /**
     * @expectedException \Lmd\Guzzle\Service\Exception\ResponseClassException
