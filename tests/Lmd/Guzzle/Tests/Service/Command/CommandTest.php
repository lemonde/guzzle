...
use Lmd\Guzzle\Plugin\Mock\MockPlugin;
use Lmd\Guzzle\Http\EntityBody;
use Lmd\Guzzle\Http\Message\Response;
use Lmd\Guzzle\Service\Client;
use Lmd\Guzzle\Service\Command\AbstractCommand;
...
use Lmd\Guzzle\Tests\Service\Mock\Command\Sub\Sub;

/**
 * @covers \Lmd\Guzzle\Service\Command\AbstractCommand
 */
class CommandTest extends AbstractCommandTest
{
    ...
    public function testExecute()
    {
        ...
        $this->assertSame($response, $command->getResponse());
        $this->assertInstanceOf('Lmd\\Guzzle\\Http\\Message\\Request', $command->getRequest());
    ...
    public function testConvertsJsonResponsesToArray()
    {
        ...
        $this->setMockResponse($client, array(
            new \Lmd\Guzzle\Http\Message\Response(200, array(
    ...
    public function testCommandsAllowsCustomRequestHeaders()
    {
        ...
        $this->assertInstanceOf('Lmd\Guzzle\Common\Collection', $command->getRequestHeaders());
    ...
    private function getOperation()
    {
        return new Operation(array(
            'name'       => 'foobar',
            'httpMethod' => 'POST',
            'class'      => 'Lmd\\Guzzle\\Tests\\Service\\Mock\\Command\\MockCommand',
            'parameters' => array(
                'test' => array(
                    'default' => '123',
                    'type'    => 'string'
    ...
    public function testHasOnCompleteMethod()
    {
        ...
        $testFunction = function($command) use (&$called, $that) {
            $called++;
            $that->assertInstanceOf('Lmd\Guzzle\Service\Command\CommandInterface', $command);
        };
    ...
    {
        $command = $this->getMockBuilder('Lmd\\Guzzle\\Service\\Command\\AbstractCommand')
    ...
    {
        $command = $this->getMockBuilder('Lmd\Guzzle\Service\Command\AbstractCommand')->getMockForAbstractClass();
        $this->assertInstanceOf('Lmd\Guzzle\Service\Description\Operation', $command->getOperation());
    }
    ...
    {
        $command = new MockCommand();
        $command->setClient(new \Lmd\Guzzle\Service\Client());
        $v = $this->getMockBuilder('Lmd\Guzzle\Service\Description\SchemaValidator')
    ...
    /**
     * @expectedException \Lmd\Guzzle\Service\Exception\ValidationException
