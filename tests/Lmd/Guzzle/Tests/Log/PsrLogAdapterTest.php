<?php

namespace Lmd\Guzzle\Tests\Log;

use Lmd\Guzzle\Log\PsrLogAdapter;
use Monolog\Logger;
use Monolog\Handler\TestHandler;

/**
 * @covers \Lmd\Guzzle\Log\PsrLogAdapter
 * @covers \Lmd\Guzzle\Log\AbstractLogAdapter
 */
class PsrLogAdapterTest extends \Lmd\Guzzle\Tests\GuzzleTestCase
{
    public function testLogsMessagesToAdaptedObject()
    {
        $log = new Logger('test');
        $handler = new TestHandler();
        $log->pushHandler($handler);
        $adapter = new PsrLogAdapter($log);
        $adapter->log('test!', LOG_INFO);
        $this->assertTrue($handler->hasInfoRecords());
        $this->assertSame($log, $adapter->getLogObject());
    }
}
