...
use Lmd\Guzzle\Plugin\Backoff\BackoffPlugin;
...
use Lmd\Guzzle\Http\Curl\CurlMultiInterface;
...
/**
 * @group server
 * @covers \Lmd\Guzzle\Plugin\Backoff\BackoffPlugin
 */
class BackoffPluginTest extends \Lmd\Guzzle\Tests\GuzzleTestCase implements EventSubscriberInterface
{
    protected $retried;
    ...
    public function testCreatesDefaultExponentialBackoffPlugin()
    {
        ...
        $this->assertInstanceOf('Lmd\Guzzle\Plugin\Backoff\BackoffPlugin', $plugin);
        $strategy = $this->readAttribute($plugin, 'strategy');
        $this->assertInstanceOf('Lmd\Guzzle\Plugin\Backoff\TruncatedBackoffStrategy', $strategy);
        $this->assertEquals(3, $this->readAttribute($strategy, 'max'));
        $strategy = $this->readAttribute($strategy, 'next');
        $this->assertInstanceOf('Lmd\Guzzle\Plugin\Backoff\HttpBackoffStrategy', $strategy);
        $this->assertEquals(array(204 => true), $this->readAttribute($strategy, 'errorCodes'));
        $strategy = $this->readAttribute($strategy, 'next');
        $this->assertInstanceOf('Lmd\Guzzle\Plugin\Backoff\CurlBackoffStrategy', $strategy);
        $this->assertEquals(array(10 => true), $this->readAttribute($strategy, 'errorCodes'));
        $strategy = $this->readAttribute($strategy, 'next');
        $this->assertInstanceOf('Lmd\Guzzle\Plugin\Backoff\ExponentialBackoffStrategy', $strategy);
    }
    ...
    public function testDoesNotRetryUnlessStrategyReturnsNumber()
    {
        ...
        $mock = $this->getMockBuilder('Lmd\Guzzle\Plugin\Backoff\BackoffStrategyInterface')
    ...
    public function testUpdatesRequestForRetry()
    {
        ...
        $handle = $this->getMockBuilder('Lmd\Guzzle\Http\Curl\CurlHandle')->disableOriginalConstructor()->getMock();
        $e = new CurlException();
    ...
    /**
     * @covers \Lmd\Guzzle\Plugin\Backoff\BackoffPlugin
     * @covers \Lmd\Guzzle\Http\Curl\CurlMulti
     */
    ...
    protected function getMockEvent(RequestInterface $request)
    {
        ...
        $multi = $this->getMockBuilder('Lmd\Guzzle\Http\Curl\CurlMulti')
