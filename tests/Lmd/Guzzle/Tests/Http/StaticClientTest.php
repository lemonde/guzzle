...
namespace Lmd\Guzzle\Tests\Plugin\Redirect;

use Lmd\Guzzle\Http\Client;
use Lmd\Guzzle\Http\StaticClient;
use Lmd\Guzzle\Plugin\Mock\MockPlugin;
use Lmd\Guzzle\Http\Message\Response;
use Lmd\Guzzle\Stream\Stream;

/**
 * @covers \Lmd\Guzzle\Http\StaticClient
 */
class StaticClientTest extends \Lmd\Guzzle\Tests\GuzzleTestCase
{
    public function testMountsClient()
    {
        ...
        $this->assertSame($client, $this->readAttribute('Lmd\Guzzle\Http\StaticClient', 'client'));
    }
    ...
    public function testSendsRequests($method)
    {
        ...
        call_user_func('Lmd\Guzzle\Http\StaticClient::' . $method, 'http://foo.com', array(
            'plugins' => array($mock)
        ));
    ...
    public function testCanCreateStreamsUsingDefaultFactory()
    {
        ...
        $this->assertInstanceOf('Lmd\Guzzle\Stream\StreamInterface', $stream);
        $this->assertEquals('test', (string) $stream);
    }

    public function testCanCreateStreamsUsingCustomFactory()
    {
        $stream = $this->getMockBuilder('Lmd\Guzzle\Stream\StreamRequestFactoryInterface')
