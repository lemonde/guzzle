...
class RequestFactory implements RequestFactoryInterface
{
    ...
    protected $requestClass = 'Lmd\\Guzzle\\Http\\Message\\Request';

    /** @var string Class to instantiate for requests with a body */
    protected $entityEnclosingRequestClass = 'Lmd\\Guzzle\\Http\\Message\\EntityEnclosingRequest';
     ...
     * {@see Lmd\Guzzle\Http\Message\Request::clone}, but can change the HTTP method.
     *
    ...
    protected function visit_exceptions(RequestInterface $request, $value, $flags)
    {
        if ($value === false || $value === 0) {
            ...
            foreach ($dispatcher->getListeners('request.error') as $listener) {
                if (is_array($listener) && $listener[0] == 'Lmd\Guzzle\Http\Message\Request' && $listener[1] = 'onRequestError') {
